...
for Task 1 (flashing external LED), Task 2 (playing Mario theme song), and Task 3 (count up every 100ms)
It demonstrates the following behavior:
  ...
  - After Task 2, start a count-down (on the 7-segment display) for 3 seconds and then restarts task 2 (original definition above) for one final time.
  - Display a “smile” for 2 seconds (See Appendix B) (code this as a new task to be controlled by Task 5). The 2 second interval begins after completion of part 5c.
...
int task_delays[] = {1000, 1000, 1, 1, 1}; // Array defining delays for tasks in milliseconds. Adjust these values to change the blinking rate.
...
void loop() {
  ...
  for (int i = 0; tasklist[i].functionptr != NULL; i++) {
    ...
    if (tasklist[i].state == STATE_READY && tasklist[i].sleeptime == 0) {
      ...
      tasklist[i].sleeptime = task_delays[i % 5]; // Assign sleep time from the predefined delays array.
